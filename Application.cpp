...
void Application::mp2Run() {
	...
	if ( par->getcurrtime() >= TEST_TIME ) {
		 ...
		 * TEST 2: Delete a non-existent key. Check for a DELETE FAIL message in the log
		 *
		 */
...
void Application::readTest() {
	...
	if ( par->getcurrtime() == (TEST_TIME + FIRST_FAIL_TIME) ) {
		...
		if ( failedOneNode ) {
			log->LOG(&mp2[nodeToFail]->getMemberNode()->addr, "TEST 2: Node failed at time=%d", par->getcurrtime());
		...
		cout<<endl<<"Reading a valid key.... ... .. . ."<<endl;
		log->LOG(&mp2[number]->getMemberNode()->addr, "TEST 2: READ OPERATION KEY: %s VALUE: %s at time: %d", it->first.c_str(), it->second.c_str(), par->getcurrtime());
		mp2[number]->clientRead(it->first);
	...
	if ( par->getcurrtime() >= (TEST_TIME + FIRST_FAIL_TIME + STABILIZE_TIME) ) {
		...
		if ( par->getcurrtime() == (TEST_TIME + FIRST_FAIL_TIME + STABILIZE_TIME) ) {
			...
			if ( count == 2 ) {
				for ( int i = 0; i < (int)nodesToFail.size(); i++ ) {
					// Fail a node
					log->LOG(&mp2[nodesToFail.at(i)]->getMemberNode()->addr, "TEST 3-1: Node failed at time=%d", par->getcurrtime());
		...
		if ( par->getcurrtime() == (TEST_TIME + FIRST_FAIL_TIME + STABILIZE_TIME + STABILIZE_TIME) ) {
            log->LOG(&mp2[number]->getMemberNode()->addr, "TEST 3-2: Read after STABILIZE_TIME at time=%d", par->getcurrtime());
			number = findARandomNodeThatIsAlive();
		...
		for ( int i = 0; i < par->EN_GPSZ; i++ ) {
			if ( !mp2[i]->getMemberNode()->bFailed ) {
				if ( mp2[i]->getMemberNode()->addr.getAddress() != replicas.at(PRIMARY).getAddress()->getAddress() &&
					...
					log->LOG(&mp2[i]->getMemberNode()->addr, "TEST 4: Node failed at time=%d", par->getcurrtime());
